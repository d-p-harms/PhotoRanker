    ...
    private func analyzeWithGemini(photoUrls: [String], criteria: RankingCriteria, completion: @escaping (Result<[RankedPhoto], Error>) -> Void) {
        ...
        functions.httpsCallable("analyzePhotos").call(data) { result, error in
            ...
            for result in results {
                ...
                if let analysisData = result["analysis"] as? [String: Any],
                   let qualityData = analysisData["quality"] as? [String: Double] {
                    
                    analysis = PhotoAnalysis(
                        quality: PhotoQuality(
                            overall: qualityData["overall"] ?? 0,
                            clarity: qualityData["clarity"] ?? 0,
                            composition: qualityData["composition"] ?? 0,
                            lighting: qualityData["lighting"] ?? 0
                        ),
                        composition: analysisData["composition"] as? String,
                        lighting: analysisData["lighting"] as? String,
                        suggestions: analysisData["suggestions"] as? [String]
                    )
                }
